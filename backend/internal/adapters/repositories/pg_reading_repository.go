...
func (r *PostgresReadingRepository) ListBySectorAndPeriod(ctx context.Context, sectorID string, start, end string) ([]domain.Reading, error) {
	var readings []domain.Reading
	query := `SELECT r.id, r.customer_id AS customerid, r.previous_value AS previousvalue, r.current_value AS currentvalue, 
	          r.consumption, r.photo_url AS photourl, r.timestamp, r.latitude, r.longitude, 
	          r.cargo_fijo AS cargofijo, r.alumbrado_publico AS alumbradopublico, 
	          r.saldo_redondeo AS saldoredondeo, r.total_to_pay AS totaltopay 
	          FROM readings r
